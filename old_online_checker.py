...
def check_ggn_online_new(d, ggn):
    ...
    for cert_type in ["GLOBAL GAP", "GRASP"]:
        ...
        if len(cert)!={}:
            link = base_link+cert["certificateId"]
            d.get(link)
            countries = d.find_elements(By.XPATH, "//div[@class='MuiBox-root css-12jz5kh']")
            if countries:
                print("yo")
                countries = countries[0].text
            valid = cert["valid"]
...
def check_ggns(ggn_list_string, tk, count_output, time_output, output):
    ...
    for idx, ggn in enumerate(ggn_list):
        ...
        start = time()
        certifs = check_ggn_online_new(d, ggn)
        output.insert(tk.END, "\n - "+str(ggn))
